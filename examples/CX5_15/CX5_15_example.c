...
int main(int argc, const char* argv[])
{
    ...
    const uint16_t sample_time = 15000;
    ...
    mip_cmd_queue_set_base_reply_timeout(queue, sample_time * 2);    
    float gyro_bias[3] = {0, 0, 0};
        
    if(mip_3dm_capture_gyro_bias(&device, sample_time, gyro_bias) != MIP_ACK_OK)
        exit_gracefully("ERROR: Could not load default device settings!");
