...
std::unique_ptr<ExampleUtils> openFromArgs(const std::string& port_or_hostname, const std::string& baud_or_port)
{
    ...
    if( port_or_hostname.find(PORT_KEY) == std::string::npos )  // Not a serial port
    {
        ...
        example_utils->connection =
            std::unique_ptr<mip::platform::TcpConnection>(new mip::platform::TcpConnection(port_or_hostname, port));
    ...
    else  // Serial port
    {
        ...
        example_utils->connection = std::unique_ptr<mip::platform::SerialConnection>(
            new mip::platform::SerialConnection(port_or_hostname, baud));
        example_utils->device     = std::unique_ptr<mip::DeviceInterface>(
            new mip::DeviceInterface(example_utils->connection.get(), example_utils->buffer,
                                     sizeof(example_utils->buffer), mip::C::mip_timeout_from_baudrate(baud), 500));
#else  // MIP_USE_SERIAL
