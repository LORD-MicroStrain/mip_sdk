...
int main(int argc, const char* argv[])
{
    try
    {
        ...
        result = mip::commands_3dm::writeMessageFormat(*device, mip::data_sensor::DESCRIPTOR_SET, descriptors.size(),
            descriptors.data());
        ...
        if (result == mip::CmdResult::NACK_COMMAND_FAILED)
        {
            ...
            result = mip::commands_3dm::writeMessageFormat(*device, mip::data_sensor::DESCRIPTOR_SET, descriptors.size() - 1,
                descriptors.data());
        }
        if (result != mip::CmdResult::ACK_OK)
        ...
        device->registerFieldCallback<&handleAccel>(dataHandlers[0], mip::data_sensor::DESCRIPTOR_SET,
