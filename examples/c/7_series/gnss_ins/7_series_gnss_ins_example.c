////////////////////////////////////////////////////////////////////////////////
/// @file 7_series_gnss_ins_example.c
///
...
#error This example requires a logging level of at least MICROSTRAIN_LOGGING_LEVEL_INFO_ to work properly
    ...
    if (!mip_cmd_result_is_ack(cmd_result))
    {
        exit_from_command(&device, cmd_result, "Could not enable the wheeled-vehicle constraint!\n");
    }
...
static void configure_gnss_message_format(mip_interface* _device, const uint8_t _gnss_data_descriptor_set)
{
    ...
    if (!mip_is_gnss_data_descriptor_set(_gnss_data_descriptor_set) || _gnss_data_descriptor_set == MIP_GNSS_DATA_DESC_SET)
    {
        exit_from_command(
            _device,
    ...
    if (!mip_cmd_result_is_ack(cmd_result))
    {
        exit_from_command(_device, cmd_result, "Could not get the base rate for GNSS %d data!\n", gnss_id);
    }
    ...
    if (!mip_cmd_result_is_ack(cmd_result))
    {
        exit_from_command(_device, cmd_result, "Could not configure message format for GNSS %d data!\n", gnss_id);
    }
}
...
static void configure_antenna_offset(mip_interface* _device, const mip_vector3f _antenna_offset, const uint8_t _antenna_id)
{
    ...
    if (!mip_cmd_result_is_ack(cmd_result))
    {
        exit_from_command(_device, cmd_result, "Could not configure the GNSS %d antenna offset!\n", _antenna_id);
    }
}
...
static void initialize_filter(mip_interface* _device)
{
    ...
    if (!mip_cmd_result_is_ack(cmd_result))
    {
        exit_from_command(_device, cmd_result, "Could not enable the aiding measurement source for GNSS heading!\n");
    }
