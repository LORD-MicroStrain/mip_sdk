...
#error This example requires a logging level of at least MICROSTRAIN_LOGGING_LEVEL_INFO_ to work properly
    ...
    while (filter_status.filter_state != MIP_FILTER_MODE_GX5_RUN_SOLUTION_VALID)
    {
        ...
        if (current_fix_type != gnss_fix_info.fix_type)
        {
            display_gnss_fix_state(&gnss_fix_info);
            current_fix_type = gnss_fix_info.fix_type;
        }
...
////////////////////////////////////////////////////////////////////////////////
/// @addtogroup _5_series_gnss_ins_example_c
/// @{
///
...
static void configure_gnss_message_format(mip_interface* _device)
{
    ...
    mip_cmd_result cmd_result = mip_3dm_gnss_get_base_rate(
        _device,
    ...
    MICROSTRAIN_LOG_INFO("Configuring message format for GNSS data.\n");
    cmd_result = mip_3dm_write_gnss_message_format(
        _device,
...
static void initialize_filter(mip_interface* _device)
{
    ...
    mip_cmd_result cmd_result = mip_filter_write_heading_source(
        _device,
        MIP_FILTER_HEADING_SOURCE_COMMAND_SOURCE_GNSS_VEL_AND_MAG // Aiding Source type
    );
...
static void display_gnss_fix_state(const mip_gnss_fix_info_data* _fix_info)
{
    ...
    switch (_fix_info->fix_type)
    {
        ...
        {
            MICROSTRAIN_LOG_INFO(
                "GNSS acquired an invalid fix. (%d) MIP_GNSS_FIX_INFO_DATA_FIX_TYPE_FIX_INVALID\n",
                fix_type_value
            );
    ...
    MICROSTRAIN_LOG_INFO("GNSS acquired a %s fix. (%d) %s\n", fix_description, fix_type_value, fix_type);

    // Confirm a valid fix was acquired
    MICROSTRAIN_LOG_INFO(
        "The current GNSS fix is %s.\n",
        (_fix_info->valid_flags & MIP_GNSS_FIX_INFO_DATA_VALID_FLAGS_FIX_TYPE) != 0 ? "valid" : "invalid"
    );
}
