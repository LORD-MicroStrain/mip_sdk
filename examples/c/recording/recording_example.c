...
#ifdef _WIN32
static const char* PORT_NAME = "COM8";
#else // Unix
...
#define CONNECTION_RECORDING_WRAPPERS false
...
#define USER_RECORDING_STREAMS false
...
bool mip_interface_user_send_to_device(mip_interface* _device, const uint8_t* _data, size_t _byte_count);
bool mip_interface_user_recv_from_device(mip_interface* _device, uint8_t* _buffer, size_t _buffer_size,
    microstrain_embedded_timestamp _wait_time, bool _from_command, size_t* _bytes_read_out, microstrain_embedded_timestamp* _timestamp_out);
...
recording_connection recording;
...
#error This example requires a logging level of at least MICROSTRAIN_LOGGING_LEVEL_INFO_ to work properly
    ...
    recording_connection_init(&recording);

    // Initialize the connection
    MICROSTRAIN_LOG_INFO("Initializing the connection on port %s with %d baudrate.\n", PORT_NAME, BAUDRATE);
    serial_port device_port;
    serial_port_init(&device_port, PORT_NAME, BAUDRATE);

    microstrain_interceptor receive_data_interceptor;
    microstrain_interceptor send_data_interceptor;

    microstrain_interceptor_handler received_data_interceptor_handler;
    microstrain_interceptor_handler sent_data_interceptor_handler;

    recording_connection_init_receive_data_interceptor(&recording, &receive_data_interceptor, &received_data_interceptor_handler);
    recording_connection_init_send_data_interceptor(&recording, &send_data_interceptor, &sent_data_interceptor_handler);

    serial_port_set_interceptors(&device_port, &receive_data_interceptor, &send_data_interceptor);
    ...
    recording_connection_init_streams(&recording, receive_stream, send_stream);
...
#else // Raw recording
    // Note: The recording interface can be used instead of the convenience functionality in the connection interfaces
    recording_connection_open_files(&recording, RECEIVED_BYTES_BINARY, SENT_BYTES_BINARY);
#endif // CONNECTION_RECORDING_WRAPPERS
    ...
    microstrain_embedded_timestamp _wait_time, bool _from_command, size_t* _bytes_read_out, microstrain_embedded_timestamp* _timestamp_out)
{
    ...
    *_timestamp_out = microstrain_get_current_timestamp();
    ...
    return serial_port_read(device_port, _buffer, _buffer_size, (uint32_t)_wait_time, _bytes_read_out, _timestamp_out);
}
...
#else // Raw recording
        if (recording_connection_enabled(&recording))
        {
            MICROSTRAIN_LOG_INFO("Closing the recording streams.\n");

            // Note: This can also be used for user-defined streams
            recording_connection_close_streams(&recording);
        }
#endif // CONNECTION_RECORDING_WRAPPERS
#endif // !USER_RECORDING_STREAMS
    }
