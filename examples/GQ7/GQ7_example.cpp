...
int main(int argc, const char* argv[])
{
    ...
    // GNSS1
    if (commands_3dm::writeMessageFormat(*device, data_gnss::MIP_GNSS1_DATA_DESC_SET, gnss_descriptors.size(),
        gnss_descriptors.data()) != CmdResult::ACK_OK)
        exit_gracefully("ERROR: Could not set GNSS1 message format!");

    // GNSS2
    if (commands_3dm::writeMessageFormat(*device, data_gnss::MIP_GNSS2_DATA_DESC_SET, gnss_descriptors.size(),
        gnss_descriptors.data()) != CmdResult::ACK_OK)
    ...
    if (commands_filter::writeAidingMeasurementEnable(*device, commands_filter::AidingMeasurementEnable::AidingSource::GNSS_POS_VEL,
        true) != CmdResult::ACK_OK)
        exit_gracefully("ERROR: Could not set filter aiding measurement enable!");

    if (commands_filter::writeAidingMeasurementEnable(*device, commands_filter::AidingMeasurementEnable::AidingSource::GNSS_HEADING,
        true) != CmdResult::ACK_OK)
        exit_gracefully("ERROR: Could not set filter aiding measurement enable!");
    ...
    if (commands_filter::writeInitializationConfiguration(*device, 0,
        commands_filter::InitializationConfiguration::InitialConditionSource::AUTO_POS_VEL_ATT,
    ...
    while (running)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            if (!gnss_fix_info_valid[i] && ((gnss_fix_info[i].fix_type == data_gnss::FixInfo::FixType::FIX_3D) ||
