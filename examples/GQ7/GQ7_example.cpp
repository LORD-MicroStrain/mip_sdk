...
int main(int argc, const char* argv[])
{
    ...
    if( commands_filter::writeAidingMeasurementEnable(*device,
                                                      commands_filter::AidingMeasurementEnable::AidingSource::GNSS_POS_VEL,
    ...
    if( commands_filter::writeAidingMeasurementEnable(*device,
                                                      commands_filter::AidingMeasurementEnable::AidingSource::GNSS_HEADING,
                                                      true) != CmdResult::ACK_OK )
    {
        exit_gracefully("ERROR: Could not set filter aiding measurement enable!");
    }
    ...
    if( commands_filter::writeInitializationConfiguration(*device, 0,
                                                          ...
                                                          alignment, 0.0f, 0.0f, 0.0f, filter_init_pos, filter_init_vel,
                                                          commands_filter::FilterReferenceFrame::LLH) !=
        CmdResult::ACK_OK )
    {
    ...
    while( running )
    {
        ...
        for( int i = 0; i < 2; i++ )
        {
            if( !gnss_fix_info_valid[i] &&
                ((gnss_fix_info[i].fix_type == data_gnss::FixInfo::FixType::FIX_3D) ||
