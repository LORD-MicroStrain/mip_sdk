...
int main(int argc, const char* argv[])
{
    ...
    if( mip_3dm_write_sensor_2_vehicle_transform_euler(&device, sensor_to_vehicle_transformation_euler[0],
                                                       ...
                                                       sensor_to_vehicle_transformation_euler[2]) != MIP_ACK_OK )
    {
    ...
    if( mip_filter_write_initialization_configuration(&device, 0,
                                                      ...
                                                      0.0f, 0.0f, 0.0f, filter_init_pos, filter_init_vel,
                                                      MIP_FILTER_REFERENCE_FRAME_LLH) != MIP_ACK_OK )
    {
    ...
    mip_interface_register_extractor(&device, &sensor_data_handlers[0], MIP_SENSOR_DATA_DESC_SET,
                                     MIP_DATA_DESC_SHARED_GPS_TIME, extract_mip_shared_gps_timestamp_data_from_field,
                                     &sensor_gps_time);
    ...
    mip_interface_register_extractor(&device, &gnss_data_handlers[0], MIP_GNSS1_DATA_DESC_SET,
                                     ...
                                     &gnss_fix_info[0]);
    mip_interface_register_extractor(&device, &gnss_data_handlers[1], MIP_GNSS2_DATA_DESC_SET,
                                     MIP_DATA_DESC_GNSS_FIX_INFO, extract_mip_gnss_fix_info_data_from_field,
                                     &gnss_fix_info[1]);
    ...
    while( running )
    {
        ...
        for( int i = 0; i < 2; i++ )
        {
            if( (gnss_fix_info_valid[i] == false) &&
                ((gnss_fix_info[i].fix_type == MIP_GNSS_FIX_INFO_DATA_FIX_TYPE_FIX_3D) ||
