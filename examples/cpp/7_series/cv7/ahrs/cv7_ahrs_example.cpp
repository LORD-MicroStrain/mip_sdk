...
void handleEventTriggers(void* _user, const mip::FieldView& _field, const mip::Timestamp _timestamp);
...
int main(const int argc, const char* argv[])
{
    ...
    device.registerExtractor(
        ...
        mip::data_filter::DESCRIPTOR_SET // Data descriptor needed for shared data
    );
...
void configureEventTriggers(mip::Interface& _device)
{
    ...
    if (!cmdResult.isAck())
    {
        terminate(cmdResult, "Could not set pitch event parameters!\n");
    }
    ...
    if (!cmdResult.isAck())
    {
        terminate(cmdResult, "Could not set roll event parameters!\n");
    }
}
...
void configureEventActions(mip::Interface& _device)
{
    ...
    if (!cmdResult.isAck())
    {
        terminate(cmdResult, "Could not set roll action parameters!\n");
    }
    ...
    if (!cmdResult.isAck())
    {
        terminate(cmdResult, "Could not set pitch action parameters!\n");
    }
}
...
void enableEvents(mip::Interface& _device)
{
    ...
    if (!cmdResult.isAck())
    {
        terminate(cmdResult, "Could not enable roll event!\n");
    }
    ...
    if (!cmdResult.isAck())
    {
        terminate(cmdResult, "Could not enable pitch event!\n");
    }
}
