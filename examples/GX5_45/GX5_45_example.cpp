...
int main(int argc, const char* argv[])
{
    ...
    if( commands_3dm::writeImuMessageFormat(*device, sensor_descriptors.size(), sensor_descriptors.data()) !=
        CmdResult::ACK_OK )
    {
    ...
    //GNSS
    if( commands_3dm::writeGpsMessageFormat(*device, gnss_descriptors.size(), gnss_descriptors.data()) !=
        CmdResult::ACK_OK )
    {
    ...
    if( commands_3dm::writeFilterMessageFormat(*device, filter_descriptors.size(), filter_descriptors.data()) !=
        CmdResult::ACK_OK )
    {
    ...
    if( commands_filter::writeSensorToVehicleRotationEuler(*device, sensor_to_vehicle_rotation_euler[0],
    ...
    if( commands_filter::writeHeadingSource(*device, commands_filter::HeadingSource::Source::GNSS_VEL_AND_MAG) !=
        CmdResult::ACK_OK )
    {
    ...
    while( running )
    {
        ...
        if( !gnss_fix_info_valid && (gnss_fix_info.fix_type == data_gnss::FixInfo::FixType::FIX_3D) &&
