...
CmdResult readSignalConfiguration(C::mip_interface& device, uint8_t* gpsEnableOut, uint8_t* glonassEnableOut, uint8_t* galileoEnableOut, uint8_t* beidouEnableOut, uint8_t* reservedOut)
{
    ...
    uint8_t responseLength = sizeof(buffer);
    CmdResult result = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_SIGNAL_CONFIGURATION, buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_SIGNAL_CONFIGURATION, buffer, &responseLength);
...
void insert(Serializer& serializer, const MipGnssSpartnConfiguration& self)
{
    ...
    if( self.function == FunctionSelector::WRITE )
    {
        ...
        insert(serializer, self.current_key_tow);
        
        insert(serializer, self.current_key_week);
        
        for(unsigned int i=0; i < 32; i++)
            insert(serializer, self.current_key[i]);
        
        insert(serializer, self.next_key_tow);
        
        insert(serializer, self.next_key_week);
        
        for(unsigned int i=0; i < 32; i++)
            insert(serializer, self.next_key[i]);
...
void extract(Serializer& serializer, MipGnssSpartnConfiguration& self)
{
    ...
    if( self.function == FunctionSelector::WRITE )
    {
        ...
        extract(serializer, self.current_key_tow);
        
        extract(serializer, self.current_key_week);
        
        for(unsigned int i=0; i < 32; i++)
            extract(serializer, self.current_key[i]);
        
        extract(serializer, self.next_key_tow);
        
        extract(serializer, self.next_key_week);
        
        for(unsigned int i=0; i < 32; i++)
            extract(serializer, self.next_key[i]);
...
CmdResult writeMipGnssSpartnConfiguration(C::mip_interface& device, uint8_t enable, uint8_t type, uint32_t currentKeyTow, uint16_t currentKeyWeek, const uint8_t* currentKey, uint32_t nextKeyTow, uint16_t nextKeyWeek, const uint8_t* nextKey)
{
    ...
    insert(serializer, currentKeyTow);
    
    insert(serializer, currentKeyWeek);
    
    assert(currentKey || (32 == 0));
    for(unsigned int i=0; i < 32; i++)
        insert(serializer, currentKey[i]);
    
    insert(serializer, nextKeyTow);
    
    insert(serializer, nextKeyWeek);
    
    assert(nextKey || (32 == 0));
    for(unsigned int i=0; i < 32; i++)
        insert(serializer, nextKey[i]);
    
    assert(serializer.isOk());
    
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_SPARTN_CONFIGURATION, buffer, (uint8_t)mip_serializer_length(&serializer));
}
CmdResult readMipGnssSpartnConfiguration(C::mip_interface& device, uint8_t* enableOut, uint8_t* typeOut, uint32_t* currentKeyTowOut, uint16_t* currentKeyWeekOut, uint8_t* currentKeyOut, uint32_t* nextKeyTowOut, uint16_t* nextKeyWeekOut, uint8_t* nextKeyOut)
{
    ...
    uint8_t responseLength = sizeof(buffer);
    CmdResult result = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_SPARTN_CONFIGURATION, buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_SPARTN_CONFIGURATION, buffer, &responseLength);
    ...
    if( result == MIP_ACK_OK )
    {
        ...
        extract(deserializer, *currentKeyTowOut);
        
        assert(currentKeyWeekOut);
        extract(deserializer, *currentKeyWeekOut);
        
        assert(currentKeyOut || (32 == 0));
        for(unsigned int i=0; i < 32; i++)
            extract(deserializer, currentKeyOut[i]);
        
        assert(nextKeyTowOut);
        extract(deserializer, *nextKeyTowOut);
        
        assert(nextKeyWeekOut);
        extract(deserializer, *nextKeyWeekOut);
        
        assert(nextKeyOut || (32 == 0));
        for(unsigned int i=0; i < 32; i++)
            extract(deserializer, nextKeyOut[i]);
...
CmdResult readRtkDongleConfiguration(C::mip_interface& device, uint8_t* enableOut, uint8_t* reservedOut)
{
    ...
    uint8_t responseLength = sizeof(buffer);
    CmdResult result = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_RTK_DONGLE_CONFIGURATION, buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_RTK_DONGLE_CONFIGURATION, buffer, &responseLength);
