...
CmdResult vehicleFrameVelWithUncAndTimestamp(C::mip_interface& device, uint8_t sourceId, double gpsTime, uint16_t gpsWeek, const float* velocity, const float* velocityUncertainty, const uint8_t* reserved)
{
    ...
    assert(velocityUncertainty || (3 == 0));
    for(unsigned int i=0; i < 3; i++)
        insert(serializer, velocityUncertainty[i]);
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_VEH_FRAME_VEL_WITH_UNC_AND_TIMESTAMP, buffer, (uint8_t)mip_serializer_length(&serializer));
}
