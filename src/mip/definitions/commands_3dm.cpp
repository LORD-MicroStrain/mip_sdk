...
CmdResult pollImuMessage(C::mip_interface& device, bool suppressAck, uint8_t numDescriptors,
    const DescriptorRate* descriptors)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_POLL_IMU_MESSAGE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult pollGnssMessage(C::mip_interface& device, bool suppressAck, uint8_t numDescriptors,
    const DescriptorRate* descriptors)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_POLL_GNSS_MESSAGE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult pollFilterMessage(C::mip_interface& device, bool suppressAck, uint8_t numDescriptors,
    const DescriptorRate* descriptors)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_POLL_FILTER_MESSAGE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult writeImuMessageFormat(C::mip_interface& device, uint8_t numDescriptors, const DescriptorRate* descriptors)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_IMU_MESSAGE_FORMAT, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readImuMessageFormat(C::mip_interface& device, uint8_t* numDescriptorsOut, uint8_t numDescriptorsOutMax,
    DescriptorRate* descriptorsOut)
{
    ...
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_IMU_MESSAGE_FORMAT,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_IMU_MESSAGE_FORMAT, buffer, &responseLength);
...
CmdResult writeGpsMessageFormat(C::mip_interface& device, uint8_t numDescriptors, const DescriptorRate* descriptors)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_GNSS_MESSAGE_FORMAT, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readGpsMessageFormat(C::mip_interface& device, uint8_t* numDescriptorsOut, uint8_t numDescriptorsOutMax,
    DescriptorRate* descriptorsOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GNSS_MESSAGE_FORMAT,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_GNSS_MESSAGE_FORMAT, buffer, &responseLength);
...
CmdResult writeFilterMessageFormat(C::mip_interface& device, uint8_t numDescriptors, const DescriptorRate* descriptors)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_FILTER_MESSAGE_FORMAT, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readFilterMessageFormat(C::mip_interface& device, uint8_t* numDescriptorsOut, uint8_t numDescriptorsOutMax,
    DescriptorRate* descriptorsOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET,
        CMD_FILTER_MESSAGE_FORMAT, buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_FILTER_MESSAGE_FORMAT,
...
CmdResult imuGetBaseRate(C::mip_interface& device, uint16_t* rateOut)
{
    ...
    CmdResult result = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GET_IMU_BASE_RATE, NULL, 0,
        REPLY_IMU_BASE_RATE, buffer, &responseLength);
...
CmdResult gpsGetBaseRate(C::mip_interface& device, uint16_t* rateOut)
{
    ...
    CmdResult result = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GET_GNSS_BASE_RATE, NULL, 0,
        REPLY_GNSS_BASE_RATE, buffer, &responseLength);
...
CmdResult filterGetBaseRate(C::mip_interface& device, uint16_t* rateOut)
{
    ...
    CmdResult result = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GET_FILTER_BASE_RATE, NULL,
        0, REPLY_FILTER_BASE_RATE, buffer, &responseLength);
...
CmdResult pollData(C::mip_interface& device, uint8_t descSet, bool suppressAck, uint8_t numDescriptors,
    const uint8_t* descriptors)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_POLL_DATA, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult getBaseRate(C::mip_interface& device, uint8_t descSet, uint16_t* rateOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GET_BASE_RATE,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_BASE_RATE, buffer, &responseLength);
...
CmdResult writeMessageFormat(C::mip_interface& device, uint8_t descSet, uint8_t numDescriptors,
    const DescriptorRate* descriptors)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_MESSAGE_FORMAT, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readMessageFormat(C::mip_interface& device, uint8_t descSet, uint8_t* numDescriptorsOut,
    uint8_t numDescriptorsOutMax, DescriptorRate* descriptorsOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_MESSAGE_FORMAT,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_MESSAGE_FORMAT, buffer, &responseLength);
...
CmdResult nmeaPollData(C::mip_interface& device, bool suppressAck, uint8_t count, const NmeaMessage* formatEntries)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_POLL_NMEA_MESSAGE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult writeNmeaMessageFormat(C::mip_interface& device, uint8_t count, const NmeaMessage* formatEntries)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_NMEA_MESSAGE_FORMAT, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readNmeaMessageFormat(C::mip_interface& device, uint8_t* countOut, uint8_t countOutMax,
    NmeaMessage* formatEntriesOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_NMEA_MESSAGE_FORMAT,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_NMEA_MESSAGE_FORMAT, buffer, &responseLength);
...
CmdResult saveDeviceSettings(C::mip_interface& device)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_DEVICE_STARTUP_SETTINGS, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult writeUartBaudrate(C::mip_interface& device, uint32_t baud)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_UART_BAUDRATE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readUartBaudrate(C::mip_interface& device, uint32_t* baudOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_UART_BAUDRATE,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_UART_BAUDRATE, buffer, &responseLength);
...
CmdResult factoryStreaming(C::mip_interface& device, FactoryStreaming::Action action, uint8_t reserved)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_CONFIGURE_FACTORY_STREAMING, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult writeDatastreamControl(C::mip_interface& device, uint8_t descSet, bool enable)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_CONTROL_DATA_STREAM, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readDatastreamControl(C::mip_interface& device, uint8_t descSet, bool* enabledOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_CONTROL_DATA_STREAM,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_DATASTREAM_ENABLE, buffer, &responseLength);
...
CmdResult writeGnssSbasSettings(C::mip_interface& device, uint8_t enableSbas, GnssSbasSettings::SBASOptions sbasOptions,
    uint8_t numIncludedPrns, const uint16_t* includedPrns)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_GNSS_SBAS_SETTINGS, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readGnssSbasSettings(C::mip_interface& device, uint8_t* enableSbasOut,
    GnssSbasSettings::SBASOptions* sbasOptionsOut, uint8_t* numIncludedPrnsOut, uint8_t numIncludedPrnsOutMax,
    uint16_t* includedPrnsOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GNSS_SBAS_SETTINGS,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_GNSS_SBAS_SETTINGS, buffer, &responseLength);
...
CmdResult writeGnssTimeAssistance(C::mip_interface& device, double tow, uint16_t weekNumber, float accuracy)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_GNSS_TIME_ASSISTANCE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readGnssTimeAssistance(C::mip_interface& device, double* towOut, uint16_t* weekNumberOut, float* accuracyOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GNSS_TIME_ASSISTANCE,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_GNSS_TIME_ASSISTANCE, buffer, &responseLength);
...
CmdResult writeImuLowpassFilter(C::mip_interface& device, uint8_t targetDescriptor, bool enable, bool manual,
    uint16_t frequency, uint8_t reserved)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_IMU_LOWPASS_FILTER, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readImuLowpassFilter(C::mip_interface& device, uint8_t targetDescriptor, bool* enableOut, bool* manualOut,
    uint16_t* frequencyOut, uint8_t* reservedOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_IMU_LOWPASS_FILTER,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_ADVANCED_DATA_FILTER, buffer, &responseLength);
...
CmdResult writePpsSource(C::mip_interface& device, PpsSource::Source source)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_PPS_SOURCE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readPpsSource(C::mip_interface& device, PpsSource::Source* sourceOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_PPS_SOURCE, buffer,
        (uint8_t)mip_serializer_length(&serializer), REPLY_PPS_SOURCE, buffer, &responseLength);
...
CmdResult writeGpioConfig(C::mip_interface& device, uint8_t pin, GpioConfig::Feature feature,
    GpioConfig::Behavior behavior, GpioConfig::PinMode pinMode)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_GPIO_CONFIG, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readGpioConfig(C::mip_interface& device, uint8_t pin, GpioConfig::Feature* featureOut,
    GpioConfig::Behavior* behaviorOut, GpioConfig::PinMode* pinModeOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GPIO_CONFIG, buffer,
        (uint8_t)mip_serializer_length(&serializer), REPLY_GPIO_CONFIG, buffer, &responseLength);
...
CmdResult writeGpioState(C::mip_interface& device, uint8_t pin, bool state)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_GPIO_STATE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readGpioState(C::mip_interface& device, uint8_t pin, bool* stateOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GPIO_STATE, buffer,
        (uint8_t)mip_serializer_length(&serializer), REPLY_GPIO_STATE, buffer, &responseLength);
...
CmdResult writeOdometer(C::mip_interface& device, Odometer::Mode mode, float scaling, float uncertainty)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_ODOMETER_CONFIG, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readOdometer(C::mip_interface& device, Odometer::Mode* modeOut, float* scalingOut, float* uncertaintyOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_ODOMETER_CONFIG,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_ODOMETER_CONFIG, buffer, &responseLength);
...
CmdResult getEventSupport(C::mip_interface& device, GetEventSupport::Query query, uint8_t* maxInstancesOut,
    uint8_t* numEntriesOut, uint8_t numEntriesOutMax, GetEventSupport::Info* entriesOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_EVENT_SUPPORT,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_EVENT_SUPPORT, buffer, &responseLength);
...
CmdResult writeEventControl(C::mip_interface& device, uint8_t instance, EventControl::Mode mode)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_EVENT_CONTROL, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readEventControl(C::mip_interface& device, uint8_t instance, EventControl::Mode* modeOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_EVENT_CONTROL,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_EVENT_CONTROL, buffer, &responseLength);
...
CmdResult getEventTriggerStatus(C::mip_interface& device, uint8_t requestedCount, const uint8_t* requestedInstances,
    uint8_t* countOut, uint8_t countOutMax, GetEventTriggerStatus::Entry* triggersOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_EVENT_TRIGGER_STATUS,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_EVENT_TRIGGER_STATUS, buffer, &responseLength);
...
CmdResult getEventActionStatus(C::mip_interface& device, uint8_t requestedCount, const uint8_t* requestedInstances,
    uint8_t* countOut, uint8_t countOutMax, GetEventActionStatus::Entry* actionsOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_EVENT_ACTION_STATUS,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_EVENT_ACTION_STATUS, buffer, &responseLength);
...
CmdResult writeEventTrigger(C::mip_interface& device, uint8_t instance, EventTrigger::Type type,
    const EventTrigger::Parameters& parameters)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_EVENT_TRIGGER_CONFIG, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readEventTrigger(C::mip_interface& device, uint8_t instance, EventTrigger::Type* typeOut,
    EventTrigger::Parameters* parametersOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_EVENT_TRIGGER_CONFIG,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_EVENT_TRIGGER_CONFIG, buffer, &responseLength);
...
CmdResult writeEventAction(C::mip_interface& device, uint8_t instance, uint8_t trigger, EventAction::Type type,
    const EventAction::Parameters& parameters)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_EVENT_ACTION_CONFIG, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readEventAction(C::mip_interface& device, uint8_t instance, uint8_t* triggerOut, EventAction::Type* typeOut,
    EventAction::Parameters* parametersOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_EVENT_ACTION_CONFIG,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_EVENT_ACTION_CONFIG, buffer, &responseLength);
...
CmdResult writeAccelBias(C::mip_interface& device, const float* bias)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_ACCEL_BIAS, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readAccelBias(C::mip_interface& device, float* biasOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_ACCEL_BIAS, buffer,
        (uint8_t)mip_serializer_length(&serializer), REPLY_ACCEL_BIAS_VECTOR, buffer, &responseLength);
...
CmdResult writeGyroBias(C::mip_interface& device, const float* bias)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_GYRO_BIAS, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readGyroBias(C::mip_interface& device, float* biasOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_GYRO_BIAS, buffer,
        (uint8_t)mip_serializer_length(&serializer), REPLY_GYRO_BIAS_VECTOR, buffer, &responseLength);
...
CmdResult captureGyroBias(C::mip_interface& device, uint16_t averagingTimeMs, float* biasOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_CAPTURE_GYRO_BIAS,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_GYRO_BIAS_VECTOR, buffer, &responseLength);
...
CmdResult writeMagHardIronOffset(C::mip_interface& device, const float* offset)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_HARD_IRON_OFFSET, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readMagHardIronOffset(C::mip_interface& device, float* offsetOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_HARD_IRON_OFFSET,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_HARD_IRON_OFFSET_VECTOR, buffer, &responseLength);
...
CmdResult writeMagSoftIronMatrix(C::mip_interface& device, const float* offset)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_SOFT_IRON_MATRIX, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readMagSoftIronMatrix(C::mip_interface& device, float* offsetOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_SOFT_IRON_MATRIX,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_SOFT_IRON_COMP_MATRIX, buffer, &responseLength);
...
CmdResult writeSensor2VehicleTransformEuler(C::mip_interface& device, float roll, float pitch, float yaw)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_SENSOR2VEHICLE_TRANSFORM_EUL, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readSensor2VehicleTransformEuler(C::mip_interface& device, float* rollOut, float* pitchOut, float* yawOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET,
        CMD_SENSOR2VEHICLE_TRANSFORM_EUL, buffer, (uint8_t)mip_serializer_length(&serializer),
        REPLY_SENSOR2VEHICLE_TRANSFORM_EUL, buffer, &responseLength);
...
CmdResult writeSensor2VehicleTransformQuaternion(C::mip_interface& device, const float* q)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_SENSOR2VEHICLE_TRANSFORM_QUAT, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readSensor2VehicleTransformQuaternion(C::mip_interface& device, float* qOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET,
        CMD_SENSOR2VEHICLE_TRANSFORM_QUAT, buffer, (uint8_t)mip_serializer_length(&serializer),
        REPLY_SENSOR2VEHICLE_TRANSFORM_QUAT, buffer, &responseLength);
...
CmdResult writeSensor2VehicleTransformDcm(C::mip_interface& device, const float* dcm)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_SENSOR2VEHICLE_TRANSFORM_DCM, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readSensor2VehicleTransformDcm(C::mip_interface& device, float* dcmOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET,
        CMD_SENSOR2VEHICLE_TRANSFORM_DCM, buffer, (uint8_t)mip_serializer_length(&serializer),
        REPLY_SENSOR2VEHICLE_TRANSFORM_DCM, buffer, &responseLength);
...
CmdResult writeComplementaryFilter(C::mip_interface& device, bool pitchRollEnable, bool headingEnable,
    float pitchRollTimeConstant, float headingTimeConstant)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_LEGACY_COMP_FILTER, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readComplementaryFilter(C::mip_interface& device, bool* pitchRollEnableOut, bool* headingEnableOut,
    float* pitchRollTimeConstantOut, float* headingTimeConstantOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_LEGACY_COMP_FILTER,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_LEGACY_COMP_FILTER, buffer, &responseLength);
...
CmdResult writeSensorRange(C::mip_interface& device, SensorRangeType sensor, uint8_t setting)
{
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_SENSOR_RANGE, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}
...
CmdResult readSensorRange(C::mip_interface& device, SensorRangeType sensor, uint8_t* settingOut)
{
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_SENSOR_RANGE, buffer,
        (uint8_t)mip_serializer_length(&serializer), REPLY_SENSOR_RANGE, buffer, &responseLength);
...
CmdResult calibratedSensorRanges(C::mip_interface& device, SensorRangeType sensor, uint8_t* numRangesOut,
    uint8_t numRangesOutMax, CalibratedSensorRanges::Entry* rangesOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_CALIBRATED_RANGES,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_CALIBRATED_RANGES, buffer, &responseLength);
...
CmdResult writeMipCmd3dmLowpassFilter(C::mip_interface& device, uint8_t descSet, uint8_t fieldDesc, bool enable,
    bool manual, float frequency)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    return mip_interface_run_command(&device, DESCRIPTOR_SET, CMD_LOWPASS_FILTER, buffer,
        (uint8_t)mip_serializer_length(&serializer));
}

CmdResult readMipCmd3dmLowpassFilter(C::mip_interface& device, uint8_t descSet, uint8_t fieldDesc, bool* enableOut,
    bool* manualOut, float* frequencyOut)
{
    uint8_t    buffer[MIP_FIELD_PAYLOAD_LENGTH_MAX];
    Serializer serializer(buffer, sizeof(buffer));
    ...
    uint8_t   responseLength = sizeof(buffer);
    CmdResult result         = mip_interface_run_command_with_response(&device, DESCRIPTOR_SET, CMD_LOWPASS_FILTER,
        buffer, (uint8_t)mip_serializer_length(&serializer), REPLY_LOWPASS_FILTER, buffer, &responseLength);
