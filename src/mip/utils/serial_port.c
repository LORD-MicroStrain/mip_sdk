...
speed_t baud_rate_to_speed(int baud_rate)
{
    switch( baud_rate )
    {
        ...
        case 230400:
            return B230400;
#ifdef __linux__ //Linux only baudrates
        case 460800:
            return B460800;
        case 500000:
            return B500000;
...
bool serial_port_open(serial_port* port, const char* port_str, int baudrate)
{
    ...
    {
        printf("\nError: Unable to open com port (%lu)\n", GetLastError());
        return false;
    }
    ...
    if( cfsetispeed(&serial_port_settings, baud_rate_to_speed(baudrate)) < 0 ||
        cfsetospeed(&serial_port_settings, baud_rate_to_speed(baudrate)) < 0 )
    {
        return false;
    }
    ...
    serial_port_settings.c_cflag =
        (serial_port_settings.c_cflag & (tcflag_t)~(CSIZE | CSTOPB | INPCK | ISTRIP | PARENB | PARODD)) | CS8;
