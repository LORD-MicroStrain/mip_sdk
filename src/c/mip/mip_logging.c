...
bool mip_format_packet_bytes(char* buffer, size_t buffer_size, size_t* index, const mip_packet_view* packet)
{
    if(!mip_packet_is_sane(packet))
        return microstrain_string_bytes_to_hex_str(buffer, buffer_size, index, mip_packet_data_pointer(packet), mip_packet_buffer_length(packet), 0);
    ...
    ok &= microstrain_string_format(
        ...
        mip_packet_data_pointer(packet)[MIP_INDEX_SYNC1],
        mip_packet_data_pointer(packet)[MIP_INDEX_SYNC2],
        mip_packet_data_pointer(packet)[MIP_INDEX_DESCSET],
        mip_packet_data_pointer(packet)[MIP_INDEX_LENGTH]
    );
...
void mip_log_packet_verbose(const mip_packet_view* packet, microstrain_log_level level)
{
    ...
    microstrain_logging_log(level, "%4s%-20s = 0x%02X\n",      " ", "MIP SYNC1", mip_packet_data_pointer(packet)[MIP_INDEX_SYNC1]);
    microstrain_logging_log(level, "%4s%-20s = 0x%02X\n",      " ", "MIP SYNC2", mip_packet_data_pointer(packet)[MIP_INDEX_SYNC2]);
