...
void serial_port_init(serial_port* _serial_port, const char* _port_name, const uint32_t _baudrate)
{
    ...
    _serial_port->data_interceptors.receive_data_interceptor  = NULL;
    _serial_port->data_interceptors.send_data_interceptor     = NULL;
}

void serial_port_set_interceptors(serial_port* _serial_port, microstrain_interceptor* _receive_interceptor,
    microstrain_interceptor* _send_interceptor)
{
    _serial_port->data_interceptors.receive_data_interceptor = _receive_interceptor;
    _serial_port->data_interceptors.send_data_interceptor    = _send_interceptor;
}
    ...
    size_t* _bytes_read_out, microstrain_embedded_timestamp* _timestamp_out)
{
    ...
    *_timestamp_out = microstrain_get_current_timestamp();
    ...
    if (serial_port_send_interceptor_enabled(_serial_port))
    {
        microstrain_interceptor_dispatch_array_data(_serial_port->data_interceptors.receive_data_interceptor,
            (void*)_buffer, _byte_count);
    ...
    size_t* _bytes_written_out)
{
    ...
    *_bytes_written_out = write(_serial_port->handle, _buffer, _byte_count);
    ...
    if (*_bytes_written_out == _byte_count)
    {
        ...
        if (serial_port_send_interceptor_enabled(_serial_port))
        {
            microstrain_interceptor_dispatch_array_data(_serial_port->data_interceptors.send_data_interceptor,
                (void*)_buffer, _byte_count);
...
// }
//
bool serial_port_receive_interceptor_enabled(const serial_port* _serial_port)
{
    if (_serial_port)
    {
        return _serial_port->data_interceptors.receive_data_interceptor != NULL;
    }
...
// }
//
bool serial_port_send_interceptor_enabled(const serial_port* _serial_port)
{
    if (_serial_port)
    {
        return _serial_port->data_interceptors.send_data_interceptor != NULL;
    }
...
// }
//
bool serial_port_interceptors_enabled(const serial_port* _serial_port)
{
    if (_serial_port)
    {
        return serial_port_receive_interceptor_enabled(_serial_port) || serial_port_send_interceptor_enabled(_serial_port);
    }
