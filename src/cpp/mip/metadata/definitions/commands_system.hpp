...
struct MetadataFor<commands_system::CommsInterface>
{
    ...
    static constexpr inline EnumInfo::Entry entries[] = {
        ...
        { uint32_t(17), "UART_1", "First configured UART. Note that this may not mean the first GPIO, if that pin is not set for UART." },
        { uint32_t(18), "UART_2", "" },
        { uint32_t(19), "UART_3", "" },
    ...
    static constexpr inline ParameterInfo parameters[] = {
        {
            ...
            /* .accessor      = */ nullptr, //utils::access<type, commands_system::CommsInterface, &type::interface>,
        ...
        {
            /* .name          = */ "protocols_in",
            /* .docs          = */ "Input protocol(s) enabled. If the protocol supports ACK/NACK or detailed responses, it will be sent over this port even if no corresponding output protocol is set.",
            /* .type          = */ {Type::BITS, &MetadataFor<commands_system::CommsProtocol>::value},
            /* .accessor      = */ nullptr, //utils::access<type, commands_system::CommsProtocol, &type::protocols_in>,
            /* .attributes    = */ {true, false, false, false, false},
            /* .count         = */ 1,
            /* .condition     = */ {},
        },
        {
            /* .name          = */ "protocols_out",
            /* .docs          = */ "Output data protocol(s) enabled.",
            /* .type          = */ {Type::BITS, &MetadataFor<commands_system::CommsProtocol>::value},
            /* .accessor      = */ nullptr, //utils::access<type, commands_system::CommsProtocol, &type::protocols_out>,
            /* .attributes    = */ {true, false, false, false, false},
            /* .count         = */ 1,
            /* .condition     = */ {},
        },
