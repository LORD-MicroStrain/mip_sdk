...
void test_create()
{
    ...
    for( unsigned int i = 0; i < sizeof(descriptors); i++ )
    {
        ...
        check(packet._buffer == buffer && packet._buffer_length == sizeof(buffer) - EXTRA,
              "mip_packet_create sets wrong buffer info");
        check(packet._buffer[MIP_INDEX_DESCSET] == descriptors[i],
              "mip_packet_create sets wrong descriptor set (%02X != %02X)", packet._buffer[MIP_INDEX_DESCSET],
              descriptors[i]);
    }
}
...
void test_add_fields()
{
    ...
    check(packet._buffer[MIP_INDEX_DESCSET] == 0x80, "Packet has wrong descriptor set (%02X != %02X)",
          packet._buffer[MIP_INDEX_DESCSET], 0x80);
    ...
    check_equal(mip_packet_total_length(&packet), MIP_PACKET_LENGTH_MIN + MIP_FIELD_HEADER_LENGTH,
                "Empty field - Total length is wrong");
    check_equal(mip_packet_payload_length(&packet), MIP_FIELD_HEADER_LENGTH,
                "Empty field - Packet payload length is wrong");
    ...
    check(mip_packet_add_field(&packet, 0x05, payload1, sizeof(payload1)), "Could not add payload of size %d",
          sizeof(payload1));
    // 7565 800A 0204 0805 010203040506 00...
    check_equal(mip_packet_total_length(&packet),
                MIP_PACKET_LENGTH_MIN + payload_size + MIP_FIELD_HEADER_LENGTH + sizeof(payload1),
                "Field 1 - Total length is wrong");
    check_equal(mip_packet_payload_length(&packet), payload_size + MIP_FIELD_HEADER_LENGTH + sizeof(payload1),
                "Field 1 - Packet payload length is wrong");
    check_equal(mip_packet_payload(&packet)[payload_size + MIP_INDEX_FIELD_DESC], 0x05,
                "Field 1 - Field descriptor is wrong");
    check_equal(mip_packet_payload(&packet)[payload_size + MIP_INDEX_FIELD_LEN], 2 + sizeof(payload1),
                "Field 1 - Field length is wrong");
    for( unsigned int j = 0; j < sizeof(payload1); j++ )
    {
        check_equal(mip_packet_payload(&packet)[payload_size + MIP_INDEX_FIELD_PAYLOAD + j], payload1[j],
                    "Field 1 - Field payload is wrong at index %d", j);
    }
    ...
    check_equal(mip_packet_alloc_field(&packet, 0x06, sizeof(payload2), &p2), 245 - 2 - sizeof(payload2),
                "Field 2 - Remaining count is wrong after allocation");
    ...
    check_equal(mip_packet_payload(&packet)[payload_size + MIP_INDEX_FIELD_DESC], 0x06,
                "Field 2 - Field descriptor is wrong");
    check_equal(mip_packet_payload(&packet)[payload_size + MIP_INDEX_FIELD_LEN], 2 + sizeof(payload2),
                "Field 2 - Field length is wrong");
...
void test_short_buffer()
{
    ...
    uint8_t* p;
    check_equal(mip_packet_alloc_field(&packet, 0x04, 3, &p), -1,
                "Wrong remaining count after allocating 1 too many bytes");
    check_equal(mip_packet_alloc_field(&packet, 0x04, MIP_FIELD_PAYLOAD_LENGTH_MAX, &p),
                2 - MIP_FIELD_PAYLOAD_LENGTH_MAX, "Wrong remaining count after allocating max payload");
    check_equal(mip_packet_alloc_field(&packet, 0x04, 255, &p), -253,
                "Wrong remaining count after allocating excessive payload");
