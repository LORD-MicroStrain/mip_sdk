...
void test_create()
{
    ...
    for (unsigned int i = 0; i < sizeof(descriptors); i++)
    {
        ...
        check(packet._buffer[MIP_INDEX_DESCSET] == descriptors[i], "mip_packet_create sets wrong descriptor set (%02X != %02X)",
            packet._buffer[MIP_INDEX_DESCSET], descriptors[i]);
    }
}
...
void test_add_fields()
{
    ...
    check_equal(mip_packet_total_length(&packet),
        MIP_PACKET_LENGTH_MIN + payload_size + MIP_FIELD_HEADER_LENGTH + sizeof(payload1), "Field 1 - Total length is wrong");
